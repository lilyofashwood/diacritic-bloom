...
The “letter-body” is an existing Unicode character; the decoration is a sequence of combining characters appended to it. The receiving font and text-layout engine place those marks around the letter. [U1]

This decorative codec makes the letter and its weather independently selectable. Seed values select repeatable visual variation.
...
The accompanying `original-specimen.txt` contains the exact unframed sequence, without an added newline. `specimen()` returns it verbatim. It preserves the original letter and mark sequence exactly.
...
The 12-mark ceiling sets the workshop's maximum density.
...
`bloom` is the default generated profile. The literal original has its own per-letter mark pattern.
...
This guarantee applies to strict-domain source text generated by the corresponding v1 encoder, with the optional frame correctly recognized. It also holds for that output after NFC/NFD in the included tests. Compatibility normalization follows the separate boundary below.
...
### Plain words and exact recipes

The plain-text decoder returns the words; the recipe retains their full history. Source symbols such as `ℝ` and `𝔄` share their plain reading with newly styled letters. Recognized accents on a styled base likewise share the decoration channel. Save a recipe to retain those source distinctions, the font choice, seed and exact mark sequence.
...
`unpack()` validates structure, version, types and limits, then regenerates the declared output and compares source and encoded strings. It rejects inconsistent recipes. Validation checks internal agreement; recipes carry no cryptographic signature, and an edited recipe is valid when its source, settings and output agree.

The JSON saves the original source openly as data. `unpack(packet).source` restores that source; `unpack(packet).encoded` restores its exact saved decoration. Import reads the packet as data without evaluating it as code.
...
The page keeps its ornamental lettering and includes a plain reading view for the decoded words. Source text, saved recipes and code-point inspection retain their literal values.

## 11. Rendering and transport

The exact code points are reproducible. Font coverage, combining-mark anchors, fallback and layout-engine support give each rendering its shape. [U1] The workshop reserves extra vertical room for tall stacks; other host layouts set their own spacing and clipping.

Keep exact specimen text in UTF-8 files or the recipe's encoded field. Keep the source and settings too. After transporting a recipe, `unpack()` checks that its saved source, settings and exact output still agree.
